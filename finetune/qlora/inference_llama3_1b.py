#!/usr/bin/env python3
"""Inference and evaluation for QLoRA-fine-tuned Llama-3.2-1B-Instruct.
...
DEFAULT_ADAPTER = Path(__file__).resolve().parent / "outputs" / "llama3_1b_adapter" / "adapter"
DEFAULT_MERGED = Path(__file__).resolve().parent / "outputs" / "llama3_1b_adapter" / "merged"
...
def load_model_and_tokenizer(
    ...
    """
    console.print("[bold cyan]Loading Llama-3.2-1B-Instruct model...[/]")

    # Load tokenizer first
    load_dir = str(merged_path) if merged_path and merged_path.exists() else str(adapter_path) if adapter_path and adapter_path.exists() else str(DEFAULT_ADAPTER)
    try:
...
def run_evaluation(
    ...
    save_path = output_path or Path(__file__).resolve().parent / "inference_llama3_1b_results.json"
...
def main() -> int:
    ...
    if not check_hf_token():
        ...
        console.print("Set HF_TOKEN or HUGGINGFACE_TOKEN environment variable.")
        console.print("Or run: huggingface-cli login")
        return 1
