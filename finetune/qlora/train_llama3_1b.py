#!/usr/bin/env python3
"""QLoRA fine-tuning of Llama-3.2-1B-Instruct for sentiment classification.

4-bit quantized LoRA adaptation of meta-llama/Llama-3.2-1B-Instruct via SFTTrainer (TRL).
Dataset: JSONL with messages format (system/user/assistant).
Target: 4-class sentiment classification (Russian reviews).

HuggingFace token REQUIRED (Llama 3.2 is a gated model).

Usage:
    python train_llama3_1b.py [--train-path PATH] [--eval-path PATH] [--output-dir DIR]
"""
...
DEFAULT_OUTPUT = Path(__file__).resolve().parent / "outputs" / "llama3_1b_adapter"
...
def load_model_quantized(model_name: str):
    ...
    console.print("[bold cyan]Loading Llama-3.2-1B-Instruct in 4-bit NF4...[/]")
...
def apply_lora(model: AutoModelForCausalLM) -> PeftModel:
    """Configure and apply LoRA adapter for Llama 3.2 architecture."""
...
def main() -> int:
    ...
    # HF token check (gated model)
    if not check_hf_token():
        console.print("[red]Error: HuggingFace token not found.[/]")
        console.print("Llama-3.2-1B-Instruct is a gated model.")
        console.print("")
        console.print("1. Get access at: https://huggingface.co/meta-llama/Llama-3.2-1B-Instruct")
        console.print("2. Generate token: https://huggingface.co/settings/tokens")
        console.print("3. Set environment variable:")
        console.print("   export HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
        console.print("   OR")
    ...
    if total_vram < 8:
        console.print(f"[yellow]Warning: {total_vram:.1f} GB VRAM is low. Minimum recommended: 8 GB.[/]")
    ...
    try:
        login(token=token)
        console.print("[green]✓ HuggingFace login successful[/]")
    except Exception as e:
        console.print(f"[yellow]Warning: HuggingFace login may have failed: {e}[/]")
        console.print("Continuing anyway — model may still cache from previous downloads.")
